...
woman=0
xout=[]
while(woman<50 or male<50):
    h=rd.randrange(120,200,1)
    w=rd.randrange(40,100,1)
    a=rd.randrange(1,100,1)
    inp=[h,w,a]
    lhs=mul(mul(trans(diff_mean),inverse(covariance)),trans([inp]))
    if (lhs[0][0]>rhs[0][0] and male<50):
        xout.append(inp + ["M"])
        male+=1
    elif (woman<50):
        xout.append(inp + ["W"])
